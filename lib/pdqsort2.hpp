...
namespace pdqsort2_detail {
    enum {
        shell_sort_threshold = 35,
    ...
    template<class Iter, class Compare>
    inline void bubble_pass(Iter begin, Iter end, Compare comp, const int gap) {
    ...
    inline void pdqsort_loop(Iter begin, Iter end, Compare comp, int bad_allowed, bool leftmost = true) {
        ...
        while (true) {
            ...
            // Shell sort is faster for small arrays.
            if (size < shell_sort_threshold) {
                bubble_pass(begin, end, comp, shell_sort_threshold / 2);
...
#if __cplusplus >= 201103L
    pdqsort2_detail::pdqsort_loop<Iter, Compare,
        pdqsort2_detail::is_default_compare<typename std::decay<Compare>::type>::value &&
        std::is_arithmetic<typename std::iterator_traits<Iter>::value_type>::value>(
        begin, end, comp, pdqsort2_detail::log2(end - begin));
#else
    pdqsort2_detail::pdqsort_loop<Iter, Compare, false>(
            begin, end, comp, pdqsort2_detail::log2(end - begin));
#endif
}
...
inline void pdqsort2_branchless(Iter begin, Iter end, Compare comp) {
    if (begin == end) return;
    pdqsort2_detail::pdqsort_loop<Iter, Compare, true>(
            begin, end, comp, pdqsort2_detail::log2(end - begin));
}
